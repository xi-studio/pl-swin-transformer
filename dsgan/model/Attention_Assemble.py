...
class CBAtten_Res(nn.Module):
    ...
    def __init__(
        ...
        self.ca = ChannelAttention(
            out_channels, reduction=self.reduction, batch_first=self.batch_first
        )

        self.sa = SpatialAttention(
            kernel_size=kernel_size, batch_first=self.batch_first
        )
    ...
    def forward(self, x):
        ...
        residual = self.max_pool(
            residual
        )  # size = (batch,in_channels,w/stride,h/stride)
        residual = self.conv_res(
            residual
        )  # size = (batch,out_channels,w/stride,h/stride)
        ...
        if not self.batch_first:
            out = out.permute(
                1, 0, 2, 3
            )  # size = (out_channels,batch,w/stride,h/stride)
...
class SKEConv(nn.Module):
    ...
    def __init__(
        self, in_channels, M=3, G=2, stride=1, L=32, reduction=2, batch_first=True
    ):
    ...
    def forward(self, x):
        ...
        for i, fc in enumerate(self.fcs):
            ...
            if i == 0:
                ...
                )  # size = (batch,M,in_channels)

        attention_vectors = self.softmax(
            attention_vectors
        )  # size = (batch,M,in_channels)
        attention_vectors = attention_vectors.unsqueeze(-1).unsqueeze(
            -1
        )  # size = (batch,M,in_channels,w,h)
...
class RecoNet(torch.nn.Module):
    ...
    def TGM_All(self, X):
        ...
        H_weight = (
            self.TGM_C(self, X.permute(0, 2, 1, 3)).permute(0, 2, 1, 3).to(X.device)
        )
        W_weight = (
            self.TGM_C(self, X.permute(0, 3, 2, 1)).permute(0, 3, 2, 1).to(X.device)
        )
...
class Self_Attn_Spatial(nn.Module):
    ...
    def __init__(self, in_dim, out_dim):
        ...
        self.query_conv = nn.Conv2d(
            in_channels=in_dim, out_channels=self.out_dim, kernel_size=1
        )
...
class Self_Attn_Channel(nn.Module):
    ...
    def forward(self, x):
        ...
        proj_key = (
            self.key_conv(x).view(m_batchsize, -1, width * height).permute(0, 2, 1)
        )
...
class GCNet_Atten(torch.nn.Module):
    ...
    def __init__(self, in_dim, reduction=4, fusion_type="mul"):
        self.in_dim = in_dim
...
class CC_module(nn.Module):
    ...
    def __init__(self, in_dim):
        ...
        self.query_conv = nn.Conv2d(
            in_channels=in_dim, out_channels=out_channels, kernel_size=1
        )
        self.key_conv = nn.Conv2d(
            in_channels=in_dim, out_channels=out_channels, kernel_size=1
        )
    ...
    def forward(self, x):
        ...
        concate = self.softmax(
            torch.cat([energy_H, energy_W], 3)
        )  # size = (b,h,w,h+w) #softmax归一化
...
class BasicConv(nn.Module):
    ...
    def __init__(
        self, in_dim, out_dim, kernel_size=3, stride=1, relu=True, bn=True, bias=False
    ):
        ...
        self.bn = (
            nn.BatchNorm2d(self.out_dim, eps=1e-5, momentum=0.01, affine=True)
            if bn
            else None
        )
...
class ChannelPool(nn.Module):
    ...
    def forward(self, x):
        ...
        max_pool_x = torch.max(x, dim=1)[0].unsqueeze(
            1
...
class AttentionConv(nn.Module):
    ...
    why? used relative positional embedding
    """
    ...
    def reset_parameters(self):
        nn.init.kaiming_normal_(
            self.key_conv.weight, mode="fan_out", nonlinearity="relu"
        )
        nn.init.kaiming_normal_(
