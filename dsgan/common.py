...
class GBlock(torch.nn.Module):
    ...
    def __init__(
        ...
        self.bn1 = torch.nn.BatchNorm2d(input_channels)  # , momentum=0.001)
        self.bn2 = torch.nn.BatchNorm2d(input_channels)  # , momentum=0.001)
        self.ln1 = torch.nn.LayerNorm([input_channels, hw, hw])  # , momentum=0.001)
        self.ln2 = torch.nn.LayerNorm([input_channels, hw, hw])  # , momentum=0.001)
...
class ContextConditioningStack(torch.nn.Module):
    def __init__(
        ...
        self.d1 = DBlock(
            ...
            output_channels=((output_channels // 4) * 1) // num_context_steps,
            conv_type=conv_type,
        )
        self.d2 = DBlock(
            input_channels=((output_channels // 4) * 1) // num_context_steps,
            output_channels=((output_channels // 2) * 1) // num_context_steps,
            conv_type=conv_type,
        )
        self.d3 = DBlock(
            input_channels=((output_channels // 2) * 1) // num_context_steps,
            output_channels=(output_channels * 1) // num_context_steps,
            conv_type=conv_type,
        )
        self.d4 = DBlock(
            input_channels=(output_channels * 1) // num_context_steps,
            output_channels=(output_channels * 2 * 1) // num_context_steps,
            conv_type=conv_type,
        )
        self.conv1 = spectral_norm(
            conv2d(
                in_channels=(output_channels // 4) * 1,
                out_channels=(output_channels // 8) * 1,
                kernel_size=3,
                padding=1,
            )
        )
        # self.conv1 = spectral_norm(
        #     conv2d(
        #         in_channels=(output_channels // 8) * 1,#input_channels,
        #         out_channels=(output_channels // 16) * 1,#input_channels,
        #         kernel_size=3,
        #         padding=1,
        #     )
        # )
        ...
        self.conv2 = spectral_norm(
            conv2d(
                in_channels=(output_channels // 2) * 1,
                out_channels=(output_channels // 4) * 1,
                kernel_size=3,
                padding=1,
            )
        )
        ...
        self.conv3 = spectral_norm(
            conv2d(
                ...
                out_channels=(output_channels // 2) * 1,
                kernel_size=3,
                padding=1,
            )
        )
    ...
    def forward(
        ...
        # print('context conditioning step input size', x.size()) #(8, 1, 4, 256, 256)
        x = self.space2depth(x)
        # print('after s2d', x.size()) #after s2d (8, 1, 16, 128, 128)
...
class LatentConditioningStack(torch.nn.Module):
    def __init__(
        ...
        self.conv_3x3 = spectral_norm(
            torch.nn.Conv2d(
                in_channels=8, out_channels=8, kernel_size=(3, 3), padding=1
            )
        )
        self.l_block1 = LBlock(input_channels=8, output_channels=output_channels // 32)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        z = torch.normal(mean=0, std=1, size=(B, 8, H // 32, W // 32)).to(
            "cpu"
        )  # (device)
...
class UpsampleGBlock4(torch.nn.Module):
    """Residual generator block with upsampling"""
    ...
    def __init__(
        ...
        self.bn1 = torch.nn.BatchNorm2d(input_channels)  # , momentum=0.001)
        self.bn2 = torch.nn.BatchNorm2d(input_channels)  # , momentum=0.001)
        self.ln1 = torch.nn.LayerNorm(input_channels)  # , momentum=0.001)
        self.ln2 = torch.nn.LayerNorm(input_channels)  # , momentum=0.001)
        self.relu = torch.nn.ReLU()
        # Upsample in the 1x1
        conv2d = get_conv_layer(conv_type)
        self.conv_1x1 = spectral_norm(
