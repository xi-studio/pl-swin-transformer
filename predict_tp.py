...
class TP(Dataset):
    ...
    def __getitem__(self, index):
        ...
        tp_t2   = 'data/era5/tp_data/rain_%05d.png' % (index + 2 + 6000)
        u_path  = 'data/era5/wind_data/wind_u_%05d.png' % (index + 1 + 6000)
        v_path  = 'data/era5/wind_data/wind_v_%05d.png' % (index + 1 + 6000)
...
def main(args, config):
    ...
    model = SwModel.load_from_checkpoint(checkpoint_path='lightning_logs/tp_4_1122/checkpoints/latest.ckpt', config=config)
    model.eval()
...
#
#    base = 0 
#    for i, (x1, x2, x3, x4, y) in enumerate(train_loader):
#        x1 = x1.to(device)
#        x2 = x2.to(device)
#        x3 = x3.to(device)
#        x4 = x4.to(device)
#        y = y.to(device)
#        yp = model(x1, x2, x3, x4)
...
#   
    ...
    np.save('data/predict_sample/obs_1122_6000_240_loss.npy', np.array(loss_list))
